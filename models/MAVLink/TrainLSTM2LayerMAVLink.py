...
#---------------------------------------------------
MAX_RUN_LEN = 46
l_paths, l_indicators = genPathsAndIndicators(final_data, [21,23,24,25,26,27,28,29,30], MAX_RUN_LEN)
...
ins = Input(shape=(None, numFeat))
#masking = layers.Masking(mask_value=0.0, input_shape=(MAX_RUN_LEN,numFeat))
masking = layers.Masking(mask_value=0.0, input_shape=(np.shape(xtrain)[1],numFeat))
h0_in = Input(shape=(32,))
c0_in = Input(shape=(32,))
h1_in = Input(shape=(32,))
c1_in = Input(shape=(32,))
d1 = layers.Dense(1, activation="sigmoid")
...
tmp0_exp, h0_out, c0_out = lstm0(m_ins, initial_state=[h0_in, c0_in])
tmp1_exp, h1_out, c1_out = lstm1(tmp0_exp, initial_state=[h1_in, c1_in])

outs_exp = d1(tmp1_exp)

model_train= Model(inputs=ins, outputs=outs)
...
model_exp = Model(inputs=[ins, h0_in, c0_in, h1_in, c1_in], outputs=[outs_exp, h0_out, c0_out, h1_out, c1_out ])


#######################################################
print("Training Now .........................")
model_base_name = '/home/users/abh61/model_mavlink_LSTM_2Layer_BBBOW_' + str(EPOCHS_BASE_ARG) + 'EP_LR_' + str(LR_ARG) + '_BATCH' + str(BATCH_SIZE_ARG) +     '_G' + str(NUM_GAMES_ARG) + '_PL' + str(PATHLEN_ARG) + '_MWNONE' 
...
class EpochSnapshotCallback(keras.callbacks.Callback):
    def on_epoch_end(self, epochs, logs=None):
        ...
        if (epochs % EPOCHS_STEP_ARG == 0):
            ...
            #mh_model_exp.save(model_base_name + '_mh_ckpt' + str(epochs))
            #path_model_exp.save(model_base_name + '_ph_ckpt' + str(epochs))
            #joint_model_exp.save(model_base_name + '_joint_ckpt' +str(epochs))
...
model_train.fit(xtrain, ytrain, epochs=EPOCHS_BASE_ARG, batch_size=BATCH_SIZE_ARG,  verbose=2, callbacks=[EpochSnapshotCallback()])
                
#mh_model_exp.save(model_base_name + '_mh')
#path_model_exp.save(model_base_name + '_ph')
#joint_model_exp.save(model_base_name + '_joint')
